...
const AddtrainingData = () => {
  ...
  const submitData = (e) => {
    ...
    axios
      .post(`${BASE_URL}/api/add-product`, formData, {
        headers: { Authorization: localStorage.getItem("token") },
      })
      .then((res) => {
        console.log("res.data", res.data, 77);
        if (res) {
          alert("Trainings Add successfully, Thanks!");
        }
        ...
        if (
          ...
          return alert("Sorry!, User Login Session has been expired");
        }
        // navigate("/");
        // props.showAlert("Service added Successfully", "success");
      })
      .catch((err) => {
        alert("Services get err");
      });
  };
  return (
    <>
